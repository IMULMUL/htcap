...
from core.crawl.lib.crawl_result import *
...
from crawl.lib.utils import adjust_requests, request_is_crawlable, request_depth, request_post_depth
from lib.exception import NotHtmlException
from lib.utils import getrealdir, get_program_infos, generate_filename, print_progressbar, stdoutw, get_phantomjs_cmd, \
	normalize_url, cmd_to_str
...
Options:
	...
	-m MODE          set crawl mode:
						- {passive}: do not interact with the page
	...
	-s SCOPE         set crawl scope
						...
						- {directory}: limit crawling to current directory (and subdirectories)
	...
	-K               keep elements in the DOM (prevent removal)""".format(
			version=infos['version'],
			passive=CRAWLMODE_PASSIVE,
			active=CRAWLMODE_ACTIVE,
			aggressive=CRAWLMODE_AGGRESSIVE,
			domain=CRAWLSCOPE_DOMAIN,
			directory=CRAWLSCOPE_DIRECTORY,
			url=CRAWLSCOPE_URL,
	...
	@staticmethod
	def generate_filename(name, out_file_overwrite):
	...
	@staticmethod
	def init_db(dbname, report_name):
		infos = {
	...
	@staticmethod
	def check_startrequest(request):
	...
	@staticmethod
	def get_requests_from_robots(request):
	...
	@staticmethod
	def check_user_script_syntax(probe_cmd, user_script):
		try:
